...
class MVN:
    ...
    def create_means(self, option, mu=0, sigma=None):
        ...
        if option == "zero":
            ...
            # Vertices of a regular (n_modes-1)-simplex centred at the origin.
            ...
            A = np.random.standard_normal((self.n_channels, self.n_modes))
            Q, _ = np.linalg.qr(A)  # Q: (M, K), orthonormal columns
            means = V @ Q.T  # (K, M)
            # Scale so std over all entries equals means_std (same role as
            # in the 'random' case).
